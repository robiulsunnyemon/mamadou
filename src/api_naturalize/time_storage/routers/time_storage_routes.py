...
async def get_time_storage(user_id: str):
    """
    FIXED: Creates TimeStorage if it doesn't exist instead of throwing 404
    Get time_storage by user ID - creates one if missing
    """
    ...
    # Try to find existing time storage
    time_storage = await TimeStorageModel.find_one(
        TimeStorageModel.user_id == db_user.id
    )
    
    # If not found, create a new one with default values
    if not time_storage:
        time_storage = TimeStorageModel(
            user_id=db_user.id,
            total_time=0
        )
