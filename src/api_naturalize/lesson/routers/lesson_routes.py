...
async def update_lesson(id: str, lesson_data: LessonUpdate):
    ...
    updated_lesson = await LessonModel.get(id)

    questions = await QuestionModel.find(
        QuestionModel.lesson_id == id
    ).to_list()

    lesson_dict = updated_lesson.model_dump()
    lesson_dict["questions"] = [q.model_dump() for q in questions]
    lesson_dict["my_progress"] = 0.0
    lesson_dict["total_right_answers"] = 0
    lesson_dict["total_questions"] = len(questions)
